...
public class Password {
    ...
    public static String bruteForce6Digit(String targetHash) {
        ...
        for (int i = 0; i <= 999999; i++) {
            String nombre = String.format("%06d", i);
            String nombre_hash = hashPassword(nombre);
            if (nombre_hash.equals(targetHash)) {
                return nombre;
            }
        }
        return null;
    ...
    public static HashMap<String, Boolean> checkPasswordsList(ArrayList<String> passwords) {
        ...
        HashMap<String, Boolean> verif = new HashMap<>();
        for (int i = 0; i < passwords.size(); i++) {
            String mdp = passwords.get(i);
            boolean strong = isStrongPassword(mdp);
            verif.put(mdp, strong);
